...
class AI_K4CRSI_V2(IStrategy):
    ...
    def feature_engineering_expand_basic(self, dataframe: DataFrame, metadata: Dict, **kwargs):
        ...
        *Only functional with FreqAI enabled strategies*
        ...
        #     # dataframe["%-ema_200-pct"] = dataframe["%-ema_200"].pct_change(periods=7)
        ...
        if metadata["tf"] == "4h":
            ...
            # dataframe["%-ema_100-pct"] = dataframe["%-ema_100"].pct_change(periods=6)
            # dataframe["%-ema_200-pct"] = dataframe["%-ema_200"].pct_change(periods=6)
            # SMA
        ...
        if metadata["tf"] == "1h":
            ...
            # dataframe["%-ema_100-pct"] = dataframe["%-ema_100"].pct_change(periods=12)
            # dataframe["%-ema_200-pct"] = dataframe["%-ema_200"].pct_change(periods=12)
        ...
        if metadata["tf"] == "15m":
            ...
            # Downtrend check
            dataframe['%-not_downtrend'] = np.where(((dataframe['close'] > dataframe['open']) | (dataframe['close'].shift(1) > dataframe['open'].shift(1)) | (dataframe['close'].shift(2) > dataframe['open'].shift(2)) | (dataframe['%-rsi_14'] > 50.0) | (dataframe['%-rsi_3'] > 25.0)),1,0)

            # Uptrend check
            dataframe['%-not_uptrend'] = np.where(((dataframe['close'] < dataframe['open']) | (dataframe['close'].shift(1) < dataframe['open'].shift(1)) | (dataframe['close'].shift(2) < dataframe['open'].shift(2)) | (dataframe['%-rsi_14'] < 50.0) | (dataframe['%-rsi_3'] < 25.0)),1,0)
